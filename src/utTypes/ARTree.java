...
public class ARTree extends UltraTree {
	...
	public void addChild(Node par, float ar, String splitDir) {
		par.addChild();
...
//	}
	...
	public void subdivideEqually( Node node, String splitDirIn, int count) {
	...
	public void subdivide(Node node, String splitDirIn, float... percents) {
		node.setVal(splitDirIn, this.splitDir);
		subdivide(node, percents);
	}

	public void subdivide(Node node, float... percents) {
...
//			children.add(new ARNode(newArs[i], this));
	...
	public float ar( Node node ) {
		return node.getVal(ar);
	}
	
	public String splitDir( Node node ) {
...
//	}
	...
	public Boundary makeBoundaryFromW( Node node, float x, float y, float w ) {
		return new Boundary( x, y, w, hFromWAR( w, ar(node) ) );
	}
	
	public List<Boundary> makeRects(  Boundary initRect, Node...node ) {
	...
	public void makeRects(Node node, List<Boundary> data ){
		if( !node.hasChildren() ) return; 
